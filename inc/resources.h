...
enum Status { FAILED = 0, PASSED = 1, WAIVED = 2 };
...
#define BENCH_START                                                            \
    Stopwatch BENCH_SW = Stopwatch();                                          \
    double BENCH_MEAN = 0.0;                                                   \
    int BENCH_I = 0, BENCH_ITER = 8;                                           \
    printf_red();                                                              \
    for (; BENCH_I < BENCH_ITER; BENCH_I++) {                                  \
        stopwatchStart(&BENCH_SW);

#define BENCH_STOP                                                             \
    stopwatchStop(&BENCH_SW);                                                  \
    printf("BENCH [%d]: %.6f sec\n", BENCH_I, BENCH_SW.elapsedTime);           \
    if (BENCH_I + 3 >= BENCH_ITER)                                             \
        BENCH_MEAN += BENCH_SW.elapsedTime;                                    \
...
inline T *MALLOC(const size_t entries,
                 const boost::optional<T *> init = (const boost::optional<T *>)
                     boost::none) {
    T *data = NULL;
#ifdef CUDA_ENABLED
    if (gpu) {
        checkCudaErrors(cuMemHostAlloc((void **)&data, entries * sizeof(T),
...
template <typename T, bool gpu = false> inline void FREE(T *&data) {
#ifdef CUDA_ENABLED
    if (gpu)
        cuMemFreeHost(data);
    else
...
inline CUdeviceptr CUDA_MALLOC(const size_t entries,
                               const boost::optional<T *> init = boost::none) {
    CUdeviceptr d = 0;
    checkCudaErrors(cuMemAlloc(&d, entries * sizeof(T)));
    if (init)
        checkCudaErrors(cuMemcpyHtoDAsync(d, (void *)init.get(),
                                          entries * sizeof(T), stream));
    else {
        if (sizeof(T) == sizeof(uint8_t))
            checkCudaErrors(cuMemsetD8Async(d, (uint8_t)val, entries, stream));
        else
            checkCudaErrors(cuMemsetD32Async(
                d, val, entries * sizeof(T) / sizeof(unsigned), stream));
    }
...
inline CUdeviceptr
CUDA_MALLOC_MANAGED(const size_t entries,
                    const boost::optional<T *> init = boost::none) {
    CUdeviceptr d = 0;
    checkCudaErrors(
        cuMemAllocManaged(&d, entries * sizeof(T), CU_MEM_ATTACH_GLOBAL));
    if (init)
...
template <typename T = int> inline void CUDA_FREE(CUdeviceptr &d) {
    cuMemFree(d);
    d = 0;
}
#endif
...
// Free a vector
template <typename T> inline void VFREE(std::vector<T> &v) {
    v.clear();
    v.shrink_to_fit();
}
...
template <typename T>
inline void
VFREE(std::vector<T, thrust::system::cuda::experimental::pinned_allocator<T>>
          &v) {
    v.clear();
    v.shrink_to_fit();
}
...
inline void connectToGPU(CuResources *cu, int argc, char **argv,
                         const int rank) {
// No null pointers
#if CU_DEBUG
    assert(cu != NULL);
...
// If that is not the case, such as for a SLURM environment with GRES
// partitions, set mpi_local_procs=1, mpi_local_rank=0
#ifdef MPI_ENABLED
    ...
    for (int j = 0; j < mpi_local_procs; j++) {
        ...
        for (unsigned i = 0; i < gpus_per_proc; i++) {
            ...
            if (version < 0x30) {
                printf("Program requires a minimum CUDA compute capability of "
                       "3.0\n");
                printFinish((const char **)argv, findExeNameStart(argv[0]), 0,
                            FAILED);
#ifdef MPI_ENABLED
            ...
            int mpcount;
            checkCudaErrors(cuDeviceGetAttribute(
                &mpcount, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                cu->cuDevice[i]));
            // Get global memory
            checkCudaErrors(
                cuDeviceTotalMem(&cu->dev_memory[i], cu->cuDevice[i]));
            printf("  Memory: %.2fGB\n  Cores: %d\n",
                   (float)cu->dev_memory[i] / 1073741824.0f,
                   mpcount * _ConvertSMVer2CoresDRV(major, minor));
            fflush(stdout);

            // Create Context
            status = cuCtxCreate(&cu->cuContext[i], CU_CTX_SCHED_SPIN,
                                 cu->cuDevice[i]);
...
template <typename T> hid_t getH5DataType();

template <> inline hid_t getH5DataType<bool>() { return H5T_NATIVE_UCHAR; }

template <> inline hid_t getH5DataType<unsigned>() { return H5T_STD_U32LE; }

template <> inline hid_t getH5DataType<int>() { return H5T_STD_I32LE; }

template <> inline hid_t getH5DataType<long>() { return H5T_STD_I64LE; }

template <> inline hid_t getH5DataType<uint64_t>() { return H5T_STD_U64LE; }

template <> inline hid_t getH5DataType<float>() { return H5T_IEEE_F32LE; }

template <> inline hid_t getH5DataType<double>() { return H5T_IEEE_F64LE; }

template <> inline hid_t getH5DataType<const char *>() {
    hid_t dt = H5Tcopy(H5T_C_S1);
    H5Tset_size(dt, H5T_VARIABLE);
    return dt;
}

inline bool write_to_h5(hid_t &handle, void *data, hsize_t dim, hsize_t *shape,
                        hsize_t *maxshape, hsize_t *chunk_size, hid_t datatype,
                        const char *dset_name, bool create, bool append) {
    ...
    if (create && append) {
        ...
        dataset = H5Dcreate(handle, dset_name, datatype, dataspace, H5P_DEFAULT,
                            props, H5P_DEFAULT);
        ...
        H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, dataspace_shape, NULL,
                            shape, NULL);
...
template <typename T>
inline void save_h5_scalar(const char *filename, const char *dataname,
                           T value) {
    ...
    if (!H5Lexists(file, dataname, 0)) {
        ...
        // Write the value
        write_to_h5(file, &value, 1, &num_entries, &max_entries, &chunk_size,
                    getH5DataType<T>(), dataname, true, true);
    } else
        write_to_h5(file, &value, 1, &num_entries, NULL, NULL,
                    getH5DataType<T>(), dataname, false, true);
...
template <typename T>
inline void save_h5_vector(const char *filename, const char *dataname,
                           const char *samplename, T *values, hsize_t *shape,
                           hsize_t dim) {
    hid_t file;
    hid_t group;
    if (access(filename, F_OK) == -1) {
        file = H5Fcreate(filename, H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
        group =
            H5Gcreate(file, dataname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    } else {
        file = H5Fopen(filename, H5F_ACC_RDWR, H5P_DEFAULT);
        if (!H5Lexists(file, dataname, 0))
            group = H5Gcreate(file, dataname, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT);
        else
            group = H5Gopen(file, dataname, H5P_DEFAULT);
    }

    write_to_h5(group, values, dim, shape, NULL, NULL, getH5DataType<T>(),
...
template <typename T>
inline void save_h5_metadata(const char *filename, const char *attrname,
                             T value) {
    ...
    if (!H5Aexists(file, attrname)) {
        ...
        hid_t attr = H5Acreate2(file, attrname, getH5DataType<T>(), attrspace,
                                H5P_DEFAULT, H5P_DEFAULT);
...
template <typename T>
inline void
save_h5_matrix_init(const char *filename, const char *dataname,
                    std::vector<std::pair<hid_t, std::string>> &datasets,
                    hid_t &file, hid_t &group, hid_t &dataspace,
                    hid_t &memspace, hsize_t rowdim[2], const hsize_t nrow,
                    const hsize_t ncol) {
    hsize_t dim = 2;
    rowdim[0] = 1;
    rowdim[1] = ncol;
    ...
    if (!group) {
        ...
        if (!exists) {
            group =
                H5Gcreate(file, dataname, lcpl_id, H5P_DEFAULT, H5P_DEFAULT);
        } else {
            group = H5Gopen(file, dataname, H5P_DEFAULT);
        }
    }
    ...
    for (size_t i = 0; i < datasets.size(); i++)
        datasets[i].first =
            H5Dcreate(group, datasets[i].second.c_str(), getH5DataType<T>(),
...
// If using MPI, call this using rank 0 *before* calling the standard init with
// all ranks
inline void save_h5_matrix_init_mpi(const char *filename, const char *dataname,
                                    hid_t &file, hid_t &group) {
#ifndef MPI_ENABLED
    fprintf(stderr,
            "Cannot call save_h5_matrix_init_mpi() when MPI is not enabled!\n");
    assert(false);
#endif
...
template <typename T>
inline void save_h5_matrix_row(const T *const rowdata, const hid_t dataset,
                               const hid_t dataspace, const hid_t memspace,
                               const hsize_t rowdim[2], hsize_t offset[2],
                               const hid_t plist) {
...
inline void
save_h5_matrix_close(hid_t &file, hid_t &group, hid_t &dataspace,
                     hid_t &memspace,
                     std::vector<std::pair<hid_t, std::string>> &datasets) {
    for (size_t i = 0; i < datasets.size(); i++)
