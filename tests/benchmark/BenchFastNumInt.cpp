...
int main(int argc, char **argv)
{
	...
	qag0 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS15, "QAG", "dat/QAG_15_Integration.dat");
	qag1 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS21, "QAG", "dat/QAG_21_Integration.dat");
	qag2 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS31, "QAG", "dat/QAG_31_Integration.dat");
	qag3 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS41, "QAG", "dat/QAG_41_Integration.dat");
	qag4 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS51, "QAG", "dat/QAG_51_Integration.dat");
	qag5 = measureFastInt(data, results, error, nnum, idata, QAG, GSL_INTEG_GAUSS61, "QAG", "dat/QAG_61_Integration.dat");
...
double measureFastInt(double * const data, double * const results, double * const error, const int &nnum, struct IntData &idata, const enum FastIntMethod fim, const int variation, const char *funcname, const char *filename)
{
	...
	if (fim == QAG || fim == QAGS || fim == QAWS) {
		...
		if (fim == QAG)
			...
			idata.table = gsl_integration_qaws_table_alloc(idata.alpha, idata.beta, idata.mu, idata.nu);
			if (idata.table == 0) {
